...
print(add_all_nums(5,4,3,2))

def convert_c_to_f (t_centigrados):
	t_fahrenheit=(t_centigrados*(9/5))+32
	return t_fahrenheit

print(convert_c_to_f(32))
...
print(calculate_slope(4,4,5,5))

def solve_quadratic_eqn(a,b,c):
	x_sol1=(-b+(b**2-4*a*c)**2)/(2*a)
	x_sol2=(-b-(b**2-4*a*c)**2)/(2*a)
	return x_sol1,x_sol2
...
print_list(["Mango","Banana","Manzana"])

def reverse_list(lista):
	list_reverse=[]
	for x in range(len(lista)-1,-1,-1):
		list_reverse.append(lista[x])
	return list_reverse

print(reverse_list([9,8,7,6,5,4,3,2,1]))

def capitalize_list_items(lista):
	capitalize=[]
	for elemento in lista:
		capitalize.append(elemento.capitalize())
	return capitalize

print(capitalize_list_items(["hola","mango","manzana","fresa"]))

def add_item(lista,agregar):
...
def median(lista):
	...
	return mediana
print(median([2,3,4,4,5,5,5,6,6]))
print(median([7,8,9,10,11,12]))

def calc_range(lista):
	lista.sort()
	range=lista[len(lista)-1]-lista[0]
	return range
print(calc_range([7,8,9,10,11,12]))
...
print(check_type([1,"mango",2,4,5]))

def check_variable(n):
	return n.isidentifier()

print(check_variable("5Hola"))
		...
		for key in data:
			if key=="languages":
				for i in range(len(data["languages"])):
					lenguajes.append(data["languages"][i])
...
def most_populated_countries(countries_data):
	poblacion=[]
	for data in countries_data:
		poblacion.append([data["population"],data["name"]])

	poblacion.sort(reverse=True)

	for i in range(10):
		print(f"{poblacion[i][1]} Es el pais # {i+1} en poblacion con {poblacion[i][0]} habitantes")
